...
/// 各GMLファイルを変換し、Kasane の建築物テーブルへ挿入します。
async fn process_gml_file(
    ...
    let spatial_ids = tokio::task::spawn_blocking(move || {
        let mut ids = Vec::new();
        match nazori::plateau::bldg::range::flat(&content, zoom, 0.0) {
            Ok(v) => {
                for ele in v {
...
pub async fn process_directory(
    ...
    pb.set_message(format!("{} [bldg] 挿入中", pref_city));
    ...
    pb.finish_with_message(format!("{} [bldg] 挿入完了", pref_city));
    Ok(())
}
